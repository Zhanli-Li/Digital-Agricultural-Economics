...
\end{enumerate}}
该部分提供了数据分析与因果推断的核心方法，是评估数字经济政策与市场行为的理论依据。
...
\end{enumerate}}
优化理论是制定数字经济中资源配置和定价策略的核心工具，保障决策过程的最优性。
...
\end{enumerate}}
该模块适用于支撑大规模在线系统的实时优化与分布式协调，提升数字平台的运行效率。

\subsection{计算机技术}
\label{sub:cs}
计算平台与编程工具是数字经济研究的数据处理和模型实现基础，涵盖编程语言、数据处理、系统管理与机器学习框架。
...
\end{enumerate}}
机器学习框架是实现智能化分析与预测的核心，支撑数字经济中自动化决策与个性化服务。

\subsection{经济学理论}
\label{sub:eco}
扎实的经济学理论是构建数字经济分析框架的支撑，有着微观、宏观与计量方法三个维度的扎实基础极大有助于数字经济研究。
...
数字经济除了需要扎实的基础知识，也需要一系列实用的技能，本文总结了一系列有益的技能，如图~\ref{fig:tech} 所示。本节首先在第\ref{sec:problem_identification}节讨论“发现问题的能力”，然后在第\ref{sec:clarify_problem}节阐述“明确问题的能力”，接着在第\ref{sec:data_collection}节介绍“信息搜集能力”，随后在第\ref{sec:reporting}节讲述“做汇报能力”，再于第\ref{sec:data_analysis}节说明“数据分析能力”，接着在第\ref{sec:interdisciplinary}节探讨“跨学科协作能力”，最后在第\ref{sec:critical_thinking}节分析“批判性思维能力”。
\begin{figure}[H]
    \centering
