...
class xLSTMModel(nn.Module):
    def __init__(self, input_size=1, hidden_size=6, output_size=1, num_layers=1):
        ...
        # 增强的门控机制部分（这里假设是通过调整LSTM的门控来实现）
        self.gate = nn.Linear(hidden_size, hidden_size)  # 可学习的门控

        # 引入改进的记忆更新
        self.memory_cell = nn.Linear(hidden_size, hidden_size)  # 用于记忆更新的线性变换
    ...
    def forward(self, x):
        ...
        # 增强的门控机制
        gate_out = torch.sigmoid(self.gate(lstm_out))  # 使用sigmoid作为门控

        # 增强的记忆更新
        memory_out = self.memory_cell(gate_out * lstm_out)  # 通过门控更新记忆

        # 全连接层处理
        out = self.fc1(memory_out)
        out = torch.relu(out)  # 使用ReLU激活函数
        out = self.fc2(out)
...
for column in data.columns:
    print(f"Training xLSTM model for {column}...")

    # 初始化模型
    model = xLSTMModel(input_size=1, hidden_size=6, output_size=1, num_layers=3)
    ...
    # 保存到Excel
    output_file = f"销量_{column}_xlstm_predictions_with_metrics.xlsx"
    with pd.ExcelWriter(output_file) as writer:
        ...
        pd.DataFrame({'训练集评价指标': ['R2', 'RMSE', 'MAPE'],
                      '值': [train_r2, train_rmse, train_mape]}).to_excel(writer, sheet_name='训练集评价指标',
                                                                          index=False)
