...
class KANLSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # KAN 部分：通过可学习的激活函数进行处理
        activated_out = torch.sigmoid(self.kernel_activation(lstm_out))  # 使用sigmoid作为激活函数
...
for column in data.columns:
    print(f"Training KAN-LSTM model for {column}...")
    ...
    # 保存到Excel
    output_file = f"销量_{column}_kan_lstm_predictions_with_metrics.xlsx"
